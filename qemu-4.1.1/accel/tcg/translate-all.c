...
#ifdef PPI_QR_FUNC
int idx;
int global_malloc_flag = 0;
extern int shm_id;
extern int shm_mem_id;
extern unsigned long int shm_mem_base_addr;
extern unsigned long int shm_base_addr;
...
#ifdef PPI_QR_FUNC
    if ( global_qrtag_flag == 2 && flush_flag == 1 )
    {
	    printf("do flush,flush flaag = %d\n",flush_flag);
	    flush_flag = 0;
	    tb_flush(cpu);
	    mmap_unlock();
	    cpu->exception_index = EXCP_INTERRUPT;
...
#ifdef PPI_QR_FUNC
    /* without -qrtag,flag = 0
       with -qrtag & use qrtag,flag = 2 */
     if ( global_qrtag_flag == 0 || global_qrtag_flag == 2 ){
	shm_offt shmoff = shm_malloc(sizeof(struct tb_trace));
	
     	tb->tt = (void *)SHM_OFFT_TO_ADDR(shmoff);
