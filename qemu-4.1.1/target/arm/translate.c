...
int tb_count;
int total_insn = 0;
int mem_transfer_num = 0;
int filter_mode = 0; // if qrtag use -p arg to trace specific program, 1 for yes, 0 for no.
int jmp_start = 0; // if qrtag use -j, to flag that jmp has started
int jmp_end_trace_start = 0; // if qrtag use -j, to flag that jmp has ended and trace is started
int max_insn = 0;
int off_insn = 0;
int regstatus = 0;
int ctrl_flag = 0;
unsigned long set_tb = 0;
unsigned long key_out = 0;
/* for get pid off */
struct tracepid gtracepid = {
    .po_current = 0UL,
#ifdef UBUNTU4
    .po_tsk = TASK_OFF,
    .po_commoff = 0x5d0,
...
void set_out(void);
void set_end(void);
#if defined(PPI_QR_FUNC) && defined(_QR_INSERT_BL_)
void ppi_insert_end_branch(DisasContext *s);
#endif

pthread_mutex_t filter_mutex;
#ifdef HAVE_HASH_LOCK
inline void ppi_lock(pthread_mutex_t *lock)
{
	pthread_mutex_lock(&filter_mutex);
	return;
}
inline void ppi_unlock(pthread_mutex_t *lock)
{
...
map pidumap = NULL,pidkmap=NULL;
extern void pidmap_out(map m);
extern void pidmap_reset(map m);
extern void pidmap_inc(map m, unsigned long pid);
#endif

#ifdef _QR_Q_TB_RECORD_
FILE *qlog=NULL;
#endif
#endif
int flush_flag = 0;
/* check current */
int valid_task(uint64_t tsk);
int valid_task(uint64_t tsk)
{
    if ( (tsk >> 48 & 0xffff) == 0xffff ) {
        return 1;
    } else
        return 0;
}
/* get CPU current task */
uint64_t get_current( CPUARMState *env);
uint64_t get_current( CPUARMState *env)
{
    uint64_t pp = 0;
...
#define THREAD_SIZE 16384
	CPUState *cpu = env_cpu(env);
        uint64_t off = 0;
        off = env->sp_el[1];
        off = off & ~(THREAD_SIZE-1);
        cpu_memory_rw_debug(cpu, (off+gtracepid.po_tsk), (uint8_t *)&pp, 8, 0);
#endif
#ifdef LINUX4
	CPUState *cpu = env_cpu(env);
        uint64_t off = 0;
        ...
        cpu_memory_rw_debug(cpu, (off+gtracepid.po_tsk), (uint8_t *)&pp, 8, 0);
#endif
#if defined(LINUX5) || defined(TEST5)
        pp  = env->sp_el[0];
#endif
...
/* get thread flags */
static inline uint32_t get_flag( CPUState *cpu,  CPUARMState *env, uint64_t current)
{
#ifdef UBUNTU4
	uint64_t flag;
        uint64_t off = 0;
        off = env->sp_el[1];
        off = off & ~(THREAD_SIZE-1);
	cpu_memory_rw_debug(cpu, (off), (uint8_t *)&flag, 8, 0);
#else
	uint32_t flag;
	cpu_memory_rw_debug(cpu, (current+24), (uint8_t *)&flag, 4, 0);
#endif
#ifdef LINUX5
	flag = bswap32(flag);
#endif
...
{
	uint32_t pid;
	cpu_memory_rw_debug(cpu, (current+gtracepid.po_off), (uint8_t *)&pid, 4, 0);
#ifdef LINUX5
	pid = bswap32(pid);
#endif
	return pid;
...
{
	uint32_t tgid;
	cpu_memory_rw_debug(cpu, (current+gtracepid.po_off+4), (uint8_t *)&tgid, 4, 0);
#ifdef LINUX5
	tgid = bswap32(tgid);
#endif
	return tgid;
...
static inline uint32_t get_ppid( CPUState *cpu,  CPUARMState *env, uint64_t current,uint32_t pid)
{
	...
	cpu_memory_rw_debug(cpu, (current+gtracepid.po_parent_off), (uint8_t *)&parent, 8, 0);
#ifdef LINUX5
	parent = bswap64(parent);
#endif
...
/* get ppid */
static inline uint32_t get_deep_ppid( CPUState *cpu,  CPUARMState *env, uint64_t current,uint32_t pid)
{
	uint32_t ppid,deep=0;
	uint64_t parent,tmp;
	gpointer presult;
        int cpuid =  cpu->cpu_index;

	ppi_lock(&filter_mutex);
	presult = g_hash_table_lookup(gtracepid.po_app[cpuid],&pid);
	if ( presult != NULL ) {
	    ppi_unlock(&filter_mutex);
            return gtracepid.po_pid;
	}
	ppi_unlock(&filter_mutex);

	tmp = current;
	do {
		cpu_memory_rw_debug(cpu, (tmp+gtracepid.po_parent_off), (uint8_t *)&parent, 8, 0);
#ifdef LINUX5
		parent = bswap64(parent);
#endif
		ppid = get_pid(cpu,env,parent);
		...
		ppi_lock(&filter_mutex);
		presult = g_hash_table_lookup(gtracepid.po_app[cpuid],&ppid);
		if ( presult != NULL ) {
	    		ppi_unlock(&filter_mutex);
            		return ppid;
		}
		...
		/* set to parent to find parent's parent*/
		tmp = parent;

		/* too deep result slow */
		if ( deep++ > 3) break;

	} while ( (ppid != 1)&&(ppid!=0) );

	    /* here add to sys */
	if ( pid <  gtracepid.po_pid ) {
		ppi_lock(&filter_mutex);
		g_hash_table_insert(gtracepid.po_sys[cpuid],&pid,&pid);
		ppi_unlock(&filter_mutex);
	}
...
static int check_out( CPUState *cpu,CPUARMState *env, uint64_t cur,int pid, int el)
{
    gpointer presult;
    int ppid=0,cpuid =  cpu->cpu_index;

    if (  gtracepid.po_flag == PO_OFF_OK ) {
        /* if in app list */
	ppi_lock(&filter_mutex);
	presult = g_hash_table_lookup(gtracepid.po_app[cpuid],&pid);
	if ( presult != NULL ) {
	...
	/* if tgid same,  qrtag have no fork, direct create_thread */
	env->ctgid = get_tgid(cpu,env,cur);
	if ( env->ctgid == gtracepid.po_tgid) {
            g_hash_table_insert(gtracepid.po_app[cpuid], &pid, &pid);
	    ppi_unlock(&filter_mutex);
	    //printf("add pid[%d] tgid[%d]to app\n",pid,env->ctgid);
            return 1;
	}

	/* rcu_sched' ppid will be app */
        if (  pid<LOW_SYSPID ) {
		/* just return */
		ppi_unlock(&filter_mutex);
                return 0;
	...
	presult = g_hash_table_lookup(gtracepid.po_sys[cpuid],&pid);
	if ( presult != NULL ) { 
		/* in sys,just return */
		ppi_unlock(&filter_mutex);
                return 0;
	...
	ppi_unlock(&filter_mutex);
        ...
        /*parent in app list*/
	if ( el == 1 ){
	    ppid = get_deep_ppid(cpu,env,cur,pid);
	}
	...
	if ( presult != NULL ) {
                g_hash_table_insert(gtracepid.po_app[cpuid], &pid, &pid);
	        ppi_unlock(&filter_mutex);
                /* add to app list, so next time will ignore */
		//printf("add pid[%d] ppid[%d]to app\n",pid,ppid);
                return 2;
	}
	ppi_unlock(&filter_mutex);
    }
...
/* get pid_off , if set, don't need */

void get_pid_off(CPUARMState *env);
void get_pid_off(CPUARMState *env)
{
    uint32_t pid=0,tgid=0,off=0;
    uint64_t pp=0,task;
    CPUState *cpu = env_cpu(env);

    if  (gtracepid.po_flag == PO_OFF_NEED) {
	    task = env->current;
            if ( valid_task(task) ==0 ) {
                //printf("invalid taskptr[%#lx]\n",task);
                return;
	    }
	    printf("Get pid of[%#lx] x20[%#lx]\n",task,env->xregs[20]);

	    /* reset po_off, if close, should set po_flag to OK, and pid_callback don't reset */
	    gtracepid.po_off = 0;
	    for ( off=100;off<1000;off++) {
		cpu_memory_rw_debug(cpu, (task+off*4), (uint8_t *)&pid, 4, 0);
		cpu_memory_rw_debug(cpu, (task+(off+1)*4), (uint8_t *)&tgid, 4, 0);
#ifdef LINUX5
		pid = bswap32(pid);
		tgid = bswap32(tgid);
#endif
		if (  (tgid == gtracepid.po_tgid) && (pid ==gtracepid.po_pid) ){
			gtracepid.po_off = off*4;
			printf("Get pid offset[%d] SUCCESS!!!\n",gtracepid.po_off);
			gtracepid.po_flag = PO_OFF_OK;
		 }
	    }
	    if ( gtracepid.po_off == 0) {
		    printf("Can't find the[%#lx] pid offset!!!\n",task);
	    } else {
		    for ( off=1;off<8;off++) {
			cpu_memory_rw_debug(cpu, (task+gtracepid.po_off+off*8), (uint8_t *)&pp, 8, 0);
#ifdef LINUX5
			pp = bswap64(pp);
#endif
			if ( valid_task(pp) ) {
			    gtracepid.po_parent_off = gtracepid.po_off +off*8;
			    printf("Get parent offset[%d]\n",gtracepid.po_parent_off);
			    break;
			}
		    }
	    }
...
#if 0
static inline int get_preempt_count( CPUARMState *env  )
{
	int ret=0;
#ifdef UBUNTU4
#define THREAD_SIZE 16384
	CPUState *cpu = env_cpu(env);
        ...
        off = env->sp_el[1];
        off = off & ~(THREAD_SIZE-1);
        cpu_memory_rw_debug(cpu, (off+24), (uint8_t *)&ret, 4, 0);
#else
	CPUState *cpu = env_cpu(env);
        uint64_t off = 0;
        off = env->sp_el[0];
#ifdef LINUX5
        cpu_memory_rw_debug(cpu, (off+16), (uint8_t *)&ret, 4, 0);
	ret = bswap32(ret);
#else
        cpu_memory_rw_debug(cpu, (off+16), (uint8_t *)&ret, 4, 0);
#endif
...
/* be called in kernel */
static inline int in_interrupt( CPUARMState *env  )
{
    int preempt_count,ret=0;

    preempt_count = get_preempt_count(env);
#ifdef TEST5
    ret = preempt_count & 0xffff00;
#else
    /* Linux5 & UBUNTU */
    ret = preempt_count & 0x1fff00;
#endif
...
/* kernel call, when switch to another process */
static inline void do_in_switch( void *s, CPUARMState *env  )
{
    CPUState *cpu = env_cpu(env);
...
#ifdef _QR_Q_TB_COUNT_
    gtracepid.po_core_switches[cpuid]++;
#endif

#ifdef  _QR_GET_OFF
    if  (gtracepid.po_flag == PO_OFF_NEED) {
        /* get from arg */
        env->current = env->xregs[1];
	if ( gtracepid.po_commoff == 0 ){
	    printf("Please get comm off first!\n");
			 
        } else {
		cpu_memory_rw_debug(cpu, (env->current+gtracepid.po_commoff), (uint8_t *)&comm, 16, 0);
		if ( (strncmp("qrtag",comm,5)==0) || (strncmp("./qrtag",comm,7)==0) ) {
			printf("Get comm[%8s] Success!\n",comm);
			/* have task->pid */
			get_pid_off(env);

			env->cpid = get_pid(cpu,env,env->current);
			env->ctgid = get_tgid(cpu,env,env->current);
	    		env->cppid = get_ppid(cpu,env,env->current,env->cpid);

			printf("GET!!! pid[%d] tgid[%d] ppid[%d] \n",env->cpid,env->ctgid,env->cppid);

			gtracepid.po_flag = PO_OFF_OK;
		}
	}
    } else if  (gtracepid.po_flag == PO_OFF_OK)
#endif
    {
	    /* get from arg */
	    /* set user env->current for next */
	    env->checkout = 0;
            env->current = env->xregs[1];
	    env->cpid = get_pid(cpu,env,env->current);
	    env->ctgid = get_tgid(cpu,env,env->current);
	    if ( filter_mode == 0 || check_out( cpu,env, env->current,env->cpid, 1) != 0 ) {
		    env->checkout = 1;
#ifdef DBU_TEMP
#define TIF_32BIT               22
	            env->cflag = get_flag(cpu,env,env->current);
		    if ( test_bit(TIF_32BIT,&env->cflag) )
		    	printf("SWITCH 32 process pid[%d] flags[%d]\n",env->cpid,env->cflag);
#endif
#ifdef COUNT_SWITCH_TB
		    printf("cpu[%d] switch tbs[%d]\n",cpuid,env->count_tbs);
		    env->count_tbs = 0;
#endif
	    } else {
		    /* filter not */
...
void helper_reg_callback ( CPUARMState *env, uint32_t reg ) {
#ifdef PPI_QR_FUNC
#ifdef PPI_QR_REG_TRACE
    if ( global_qrtag_flag == 0 || global_qrtag_flag == 2 ){
        if ( env->needsend ) {
            if ( env->sendbuf->reg_num >= MAX_REGINFOS ) {
                printf("reg number[%d]\n", env->sendbuf->reg_num);
            } else {
                env->sendbuf->ri[env->sendbuf->reg_num] = reg;
                env->sendbuf->reg_num++;
            }
        }
    }
...
void helper_load_callback( CPUARMState *env,uint64_t addr )
{
    struct mem_info *pinfo;
    if ( global_qrtag_flag == 0 || global_qrtag_flag == 2 ){
             /* load mem */
            if ( env->needsend && env->mem_need_send == 1 ) {
                if ( env->sendbuf->mem_num >= MAX_MEMINFOS ) {
                        // printf("mem num[%d]\n",env->sendbuf->mem_num);
                } else {
                   pinfo = &(env->sendbuf->mi[env->sendbuf->mem_num]);
                   env->sendbuf->mem_num++;
                   pinfo->type = MEM_LOAD;
#ifdef PPI_QR_FUNC
#ifdef PPI_QR_MEM
                   if(env->process_tb != NULL){
                        int mmu_idx = cpu_mmu_index(env,false);
                        void *hva = tlb_vaddr_to_host(env,addr,MMU_DATA_LOAD,mmu_idx);
                        if(hva == NULL)
                          pinfo->addr = addr;
                        else
                          pinfo->addr = hva;
...
void helper_store_callback( CPUARMState *env,uint64_t addr )
{
    struct mem_info *pinfo;
    if ( global_qrtag_flag == 0 || global_qrtag_flag == 2 ){
              /* store mem */
            if ( env->needsend && env->mem_need_send == 1 ) {
                if ( env->sendbuf->mem_num >= MAX_MEMINFOS ) {
                    ...
                    pinfo = &(env->sendbuf->mi[env->sendbuf->mem_num]);
                    env->sendbuf->mem_num++;
                    pinfo->type = MEM_STORE;
#ifdef PPI_QR_FUNC
#ifdef PPI_QR_MEM
                    if(env->process_tb != NULL){
                        int mmu_idx = cpu_mmu_index(env,false);
                        void *hva = tlb_vaddr_to_host(env,addr,MMU_DATA_STORE,mmu_idx);
                        if(hva == NULL)
                          pinfo->addr = addr;
                        else
                          pinfo->addr = hva;
...
void helper_start_callback( void *s, CPUARMState *env  )
{
...
#ifdef LINUX5 /* for HW, out when ctl+S */
    if (key_out == 0) {
	return;
    }
#endif
    ...
    if (tb->tt != NULL) {
	int cpuid =  cpu->cpu_index;
#ifdef _QR_Q_TB_COUNTALL_
	/* count all */
	env->needcount  = 1;
#endif

	/* reduce checkout distinguish */
	if ( (env->checkout == 0) && key_out != 2 ) {
		return;
	}

#ifdef  _QR_GET_OFF
    	if  ( gtracepid.po_flag != PO_OFF_OK ) {
		/* without off, wrong pid&ppid */
		return;
    	}
#endif
	/* get current task */
	eltype = arm_current_el(env);
	/* eltype may change by tb ins, msr etc */
	env->celtype = eltype;
#if 0
	/* exclude interrupt, all is process */
	if ( eltype==1 ) {
		if ( in_interrupt(env) ) {
			/* interrupt */
			return;
    ...
    // deal with -j -n
    int out_status = 0;
    if (env->checkout == 1 && regstatus == 1) {
        if (max_insn != 0) {
            if (off_insn != 0) {  // both j and n
                if (total_insn < off_insn) {
                    if (jmp_start == 0) {
                        if (atomic_inc_fetch(&jmp_start) == 1) {
                            printf("QEMU JUMP START\n");
                        }
                    }
                } else if (total_insn >= off_insn && total_insn < max_insn + off_insn) {
                    out_status = 1;
                    if (jmp_end_trace_start == 0) {
                        if (atomic_inc_fetch(&jmp_end_trace_start) == 1) {
                            printf("QEMU JUMP END AND TRACE START\n");
                        }
                    }
                } else if (total_insn > max_insn + off_insn) {
                    printf("QEMU TRACE END BY INSN COUNT: %d\n", total_insn);
                    set_end();
                }
            } else {  // only n
                if (total_insn < max_insn) {
                    out_status = 1;
                    if (jmp_end_trace_start == 0) {
                        if (atomic_inc_fetch(&jmp_end_trace_start) == 1) {
                            printf("QEMU TRACE START\n");
                        }
                    }
                } else {
                    printf("QEMU TRACE END BY INSN COUNT: %d\n", total_insn);
        ...
        atomic_add(&total_insn, tb->tt->insn_num);
    }
    if ( regstatus == 1 && out_status == 0) { // skip by -j
        return;
    }

    if ( (regstatus ==1 && out_status == 1)  || key_out == 1 || (key_out==2) ){
		...
		env->count_pid = pid;
		env->count_status = 'N';
		if ( atomic_read(&set_tb) == 100 ){
			if ( atomic_inc_fetch(&set_tb) == 101 ){
				env->count_status = 'S';
				atomic_set(&set_tb,0);
			} 
		} else if (  atomic_read(&set_tb) == 1000 ){
			if (atomic_inc_fetch(&set_tb) == 1001 ) {
				
				env->count_status = 'E';
				atomic_set(&set_tb,0);
				atomic_set(&key_out,0);
			}
		}
#ifdef _QR_Q_TB_COUNT_
		env->needcount  = 1;
#ifdef _QR_DEBUG_INFO_
		if ( tb->pc > 0x510000 ) {
			uint32_t code;
			cpu_memory_rw_debug(cpu, 0x400458, (uint8_t *)&code, 4, 0);
			printf("pc[%#x] code[%#x]\n",tb->pc,code);
		}
#endif
#endif
...
#ifdef _QR_QR_RIO_
		/* get idx & set needsend */
    			if ( env->needsend == 0  ) {
            			while (true){
                    			env->shm_index = ringbuf_get_index(sizeof(struct qrio_buff),  cpu->cpu_index);
		...
		env->sendbuf = shm_getaddr_from_idx(env->shm_index);
		env->sendbuf->mem_num = 0;
        env->sendbuf->reg_num = 0;


		struct qrio_buff *buf = env->sendbuf;

		if ( env->count_status == 'S') {
			shm->app_status = 1;
		} else if ( env->count_status == 'E' ) {
			shm->app_status = 2;
		}
		buf->insn_type = eltype;
		if ( eltype == 1 ) {
                		/* reload pid*/
                		buf->pid = pid;
                		buf->tgid = tgid;
		...
		buf->cpu_id = cpuid;
		buf->current = current;	
		buf->offset = SHM_ADDR_TO_OFFT(tb->tt);	
...
void helper_end_callback( void *s, CPUARMState *env , uint32_t from_cpu_loop_exit , uint32_t real_insn_num ) // only use real_insn_num when from_cpu_loop_exit is 1
{

	int eltype = env->celtype;
#ifdef _QR_Q_TB_COUNT_
        CPUState *cpu = env_cpu(env);
	int cpuid =  cpu->cpu_index;
	if ( (env->needsend == 0)&&(env->needcount == 0) )
		return;
#else
	if ( env->needsend == 0 )
		return;
	...
	if ( env->needsend  == 1 ) {
#ifdef DEBUG_SWITCH
		if ( i_end ++ < 10 )
			// printf("end_callback: mem num = %d\n",env->sendbuf->mem_num);
#endif
        if( env->insn_need_send == 1 ){
            if (from_cpu_loop_exit==1) {
                if(  (int) real_insn_num >=0 && (int)real_insn_num < tb->tt->insn_num){
                    env->sendbuf->real_insn_num = (int) real_insn_num;
                }
                else{
                    env->sendbuf->real_insn_num = tb->tt->insn_num;
                }
            }
            else {
                env->sendbuf->real_insn_num = tb->tt->insn_num;
		...
		while(1){
			int ret = ringbuf_push_new( env->shm_index);
			if ( ret == 0) {
				break;
			}
		}

		/* reset needsend, set only have 1 */
		env->needsend = 0;
		env->sendbuf = NULL;
	}
#endif
	...
	if ( env->needcount ) {
	    if ( eltype == 1) {
		gtracepid.po_core_ktb[cpuid]++;
#ifdef _QR_Q_PIDMAP_
		if ( pidkmap != NULL ) {
			pidmap_inc( pidkmap, env->count_pid);
		}
#endif
		gtracepid.po_core_kins[cpuid] +=  tb->tt->insn_num;

	    } else if ( eltype == 0 ) {
		gtracepid.po_core_utb[cpuid]++;
#ifdef _QR_Q_PIDMAP_
		if ( pidumap != NULL ) {
			pidmap_inc(pidumap, env->count_pid);
		}
#endif
		if (from_cpu_loop_exit==1) {
			gtracepid.po_core_uins[cpuid] += real_insn_num;
		} else
			gtracepid.po_core_uins[cpuid] +=  tb->tt->insn_num;
#ifdef _QR_Q_TB_RECORD_
		if ( qlog !=NULL )
			fprintf(qlog,"%#lx\n", tb->pc);
#endif
	    }
	    if ( from_cpu_loop_exit )
		 gtracepid.po_core_excepts[cpuid]++;
...
#ifdef _QR_Q_PIDMAP_
void pidmap_out(map m)
{
     for (const char *key = map_first(m); key != NULL; key = map_next(m, key)) {
                    printf("%s: %ld \n", key, *(uint64_t *) map_get(m, key));
     }
     return;

}
void pidmap_reset(map m)
{

     for (const char *key = map_first(m); key != NULL; key = map_next(m, key)) {
                *(uint64_t *) map_get(m, key) = 0;
     }
     return;

}
void pidmap_inc(map m, unsigned long pid)
{
	pthread_mutex_lock(&mutex);  
        ...
        if (map_contains(m, pidstr)) {
                *(uint64_t *) map_get(m, pidstr) += 1;
        } else {
...
#ifdef LINUX5
	printf("--------LINUX5:set_out----------\n");
#endif
#ifdef UBUNTU4
	printf("--------UBUNTU4:set_out----------\n");
#endif
	if ( global_qrtag_flag == 1 ){
                global_qrtag_flag = 2;
        }	
	tb_count = 0;
	total_insn = 0;
	...
	if ( pidumap == NULL ){
		...
		/*
    		map oldumap,oldkmap;
                ...
                map_destroy(oldkmap);
		*/
		pidmap_reset(pidumap);
		pidmap_reset(pidkmap);
        }
	pthread_mutex_unlock(&mutex);  
#endif

	memset( gtracepid.po_core_utb,0,PPI_CORES*sizeof(uint64_t));
	memset( gtracepid.po_core_ktb,0,PPI_CORES*sizeof(uint64_t));
	memset( gtracepid.po_core_kins,0,PPI_CORES*sizeof(uint64_t));
	memset( gtracepid.po_core_uins,0,PPI_CORES*sizeof(uint64_t));
	memset( gtracepid.po_core_excepts,0,PPI_CORES*sizeof(uint64_t));
	memset( gtracepid.po_core_switches,0,PPI_CORES*sizeof(uint64_t));

	curtime = time(NULL);
	info = localtime( &curtime );
	sprintf(logname,"utb-%d-%d-%d-%d.log",
			info->tm_mday,
			info->tm_hour,
			info->tm_min,
			info->tm_sec);
#ifdef _QR_Q_TB_RECORD_
	qlog = fopen(logname,"w+");
#endif

#endif

	atomic_set(&set_tb,100);
	atomic_set(&key_out,1);
	qemu_clock_enter_sonic();
	flush_flag = 1;
}	
void set_end(void)
{
    ...
    tb_count = 0;
    jmp_start = 0;
    jmp_end_trace_start = 0;
	fflush(stdout);
	atomic_set(&set_tb,1000);
	atomic_set(&key_out,2);
	qemu_clock_exit_sonic();
...
#ifdef _QR_Q_TB_COUNT_
void tb_count_out( void)
{
	uint64_t sumutb=0,sumktb=0,sumkins=0,sumuins=0,sumexcepts=0,sumswitches=0;

	for (int i = 0;i<16;i++)
	{
		sumutb += gtracepid.po_core_utb[i];
		sumktb += gtracepid.po_core_ktb[i];
		sumkins +=gtracepid.po_core_kins[i];
		sumuins +=gtracepid.po_core_uins[i];
		sumexcepts += gtracepid.po_core_excepts[i];
		sumswitches += gtracepid.po_core_switches[i];
		printf("Core[%d] uins[%lu],kins[%lu],exceptions[%lu],switches[%lu]\n", i,
				gtracepid.po_core_uins[i],
				gtracepid.po_core_kins[i],
				gtracepid.po_core_excepts[i],
				gtracepid.po_core_switches[i]);
	}
	printf("Total  uins[%lu],kins[%lu],exceptions[%lu],switches[%lu]\n",
				sumuins,sumkins,sumexcepts,sumswitches);

}
#endif
void helper_map_callback( void *s, CPUARMState *env)
{
	unsigned long data_qrtag = env->xregs[8];
	CPUState *cpu = env_cpu(env);

	char *data_qemu = (char *)malloc(sizeof(char)*20);	
	cpu_memory_rw_debug(cpu, data_qrtag, (uint8_t *)data_qemu, 20, 0);

	char *offset_path= (char *)malloc(sizeof(char)*100);
	sprintf(offset_path,"%s/qr/%d/dynamic_link_offset.txt",getenv("HOME"),channel_d);
	FILE* f_offset = fopen(offset_path, "w");
	fprintf(f_offset, "%s", data_qemu);
	fclose(f_offset);
	return;
}
void helper_pid_callback( void *s, CPUARMState *env)
{
...
#ifdef _QR_Q_TB_COUNT_
	printf("come into pid_callback\n");
#endif
        /* pid is tid, tgid is pid */
   	gtracepid.po_ppid = env->xregs[8];
	gtracepid.po_tgid = env->xregs[9];
	gtracepid.po_pid = env->xregs[10];
	/* comm it when off is definitly*/
	//gtracepid.po_off = 0;  
#ifdef _QR_GET_OFF
	gtracepid.po_flag = PO_OFF_NEED;
#else
	gtracepid.po_flag = PO_OFF_OK;
#endif
	/* set to filter */
	...
	for ( idx=0; idx<PPI_CORES; idx++) {
		g_hash_table_remove_all(gtracepid.po_app[idx]);
		g_hash_table_remove_all(gtracepid.po_sys[idx]);
		g_hash_table_insert(gtracepid.po_app[idx],&gtracepid.po_tgid,&gtracepid.po_tgid);
		//g_hash_table_insert(gtracepid.po_app[idx],&gtracepid.po_pid,&gtracepid.po_pid);
	}
...
//#ifdef TEST5
	/* first tb out */
	env->checkout = 1;
	env->cpid = gtracepid.po_pid;
	env->ctgid = gtracepid.po_tgid;
//#endif

#ifdef _QR_Q_TB_COUNT_
	printf("pid align ppid[%d] pid [%d] tgid[%d] \n",gtracepid.po_ppid,gtracepid.po_pid,gtracepid.po_tgid);
#endif

    return;
 	
}
void helper_outreg_callback( void *s, CPUARMState *env)
{

    /* get max & off insn */
	max_insn = env->xregs[8];
	off_insn = env->xregs[9];
    /* start trace */
	qemu_clock_enter_rate();
    filter_mode = 1;
#ifndef LINUX5
    	set_out();
#endif
...
#ifdef _QR_Q_TB_COUNT_
	printf("outreg max_insn =  %d\n",max_insn);
	printf("outreg off_insn =  %d\n",off_insn);
#endif
 	
}

void helper_outreg_end_callback( void *s, CPUARMState *env)
{
	printf("into outreg_end_callback\n");

	/* qrtag wait child exit and indicate end */
#ifndef LINUX5
    if (regstatus == 1) { // may stop by qrtag -n (insn count) already.
        set_end();
    }
#endif
	qemu_clock_exit_rate();
#ifdef _QR_Q_TB_COUNT_
#ifdef _QR_Q_TB_RECORD_
	fflush(qlog);
	fclose(qlog);
...
void ppi_insert_end_branch(DisasContext *s)
{
	if ( global_qrtag_flag == 0 || global_qrtag_flag == 2 ){
	    ...
	    TCGv_i32 from_cpu_loop_exit;
	    TCGv_i32 real_insn_num;
	    from_cpu_loop_exit = tcg_const_i32(0);
	    real_insn_num = tcg_const_i32(0);
		
            gen_helper_end_callback(dcs, cpu_env ,from_cpu_loop_exit , real_insn_num);
	    tcg_gen_st_i32(tcg_const_i32(0),cpu_env,offsetof(CPUArchState , num));
            tcg_temp_free_ptr(dcs);
            tcg_temp_free_i32(from_cpu_loop_exit);
	    tcg_temp_free_i32(real_insn_num);
	}
	return;
}
#else
...
#ifdef HAVE_SWITCH
        if ( dcbase->pc_first == HAVE_SWITCH ) {
...
#ifdef PPI_QR_FUNC
    if ( global_qrtag_flag == 0 || global_qrtag_flag == 2 ){
    	struct tb_trace *ptt = dcbase->tb->tt;
    	ptt->ti[ptt->insn_num].pc = dc->pc;
    	ptt->ti[ptt->insn_num].instr = dc->insn;
    	ptt->insn_num++;	
    }
#endif
...
#ifdef PPI_QR_FUNC
    if ( global_qrtag_flag == 0 || global_qrtag_flag == 2 ){
    	struct tb_trace *ptt = dc->base.tb->tt;
    	ptt->ti[ptt->insn_num].pc = dc->pc;
    	ptt->ti[ptt->insn_num].instr = dc->insn;
    	ptt->insn_num++;	
    }
