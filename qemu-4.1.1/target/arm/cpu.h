...
typedef struct CPUARMState {
    ...
    uint64_t xregs[32];
    uint64_t pc;
    //unsigned char* pre_jmp_pc; //add filed
...
#define FFR_PRED_NUM 16
    ...
    // Control whether meminfo needs to be passed
    int mem_need_send;
    int insn_need_send;
    ...
    uint64_t current;
    uint64_t mmu;
    struct TranslationBlock *process_tb;
    struct qrio_buff *sendbuf;
    int tb_finish_flag;
    int shm_index;
    char     count_status;
    uint32_t count_pid;
...
#ifdef PPI_QR_FUNC
#define PO_OFF_NEED 0x01
#define PO_OFF_OK   0x02
#define PPI_CORES   64     /* max cores */
#define PPI_MAX_REGS   10  /* max switch time when reg */
...
struct tracepid{
    ...
    uint32_t po_tgid;
    uint32_t po_flag;
    uint64_t po_core_utb[PPI_CORES];
    uint64_t po_core_ktb[PPI_CORES];
    uint64_t po_core_kins[PPI_CORES];
    uint64_t po_core_uins[PPI_CORES];
    uint64_t po_core_excepts[PPI_CORES];
    uint64_t po_core_switches[PPI_CORES];
    GHashTable *po_app[PPI_CORES];
    GHashTable *po_sys[PPI_CORES];
};
#endif
